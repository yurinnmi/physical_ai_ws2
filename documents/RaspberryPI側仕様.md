...
### 3.3 複数物体検出モード (teddy bear / cup / bottle)

`config.py` の `ENABLE_MULTI_OBJECT_DETECTION` を `True` にすると、人物検出の代わりに以下3種類を検出するモードで動作する（`False`で従来の人物検出モードに戻る。ロジックは変更していない）。

| 対象 | COCOクラスID (`yolov8n.pt`) |
|---|---|
| teddy bear | 77 |
| cup | 41 |
| bottle | 39 |

安定化条件（3フレーム連続検出でON、2秒未検出でOFF）は対象ごとに独立して適用し、状態が変化した対象のみM5Stackへ通知する。3つ同時に検出された場合は3つとも通知され、M5Stack側で同時表示できる。
